...
def test_name_normalizes_capitalization_and_spaces():
    ...
    datos, errores = validar_usuario(
        "   JoSé      MuÑOz   ", "test@gmail.com", "Enana09!", True, True
    )
    assert not errores
    assert datos["name"] == "José Muñoz"
...
def test_name_with_multiple_words():
    ...
    datos, errores = validar_usuario(
        "María José López García", "test@gmail.com", "Enana09!", True, True
    )
    assert not errores
...
def test_email_maximum_length():
    ...
    _, errores = validar_usuario(
        "Juan Pérez", f"{long_local_part}@gmail.com", "Segura2026", True, True
    )
    assert "email" in errores
