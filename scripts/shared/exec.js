...
const resolveCommand = (command, args) => {
    if (isWindows && (command === 'npm' || command === 'npx' || command === 'npm.cmd' || command === 'npx.cmd')) {
        const baseCmd = command.endsWith('.cmd') ? command : `${command}.cmd`
        return {
            cmd: 'cmd.exe',
...
const runCommand = ({ command, args = [], cwd, env, silent = false }) => {
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
            if (code !== 0) {
                const err = new Error(`${command} terminó con código ${code}`)

                if (silent && output.trim()) {
                    const lines = output.trim().split('\n')
                    err.details = lines.slice(-25).join('\n')
                }
...
const runCommandCapture = ({ command, args = [], cwd, env }) => {
    return new Promise((resolve, reject) => {
        const { cmd, resolvedArgs } = resolveCommand(command, args)

        execFile(cmd, resolvedArgs, {
            cwd,
            env: env ? { ...process.env, ...env } : process.env,
            shell: false
        }, (error, stdout, stderr) => {
